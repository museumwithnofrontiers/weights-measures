...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('weightsMeasures.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('weightsMeasures.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('weightsMeasures.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
